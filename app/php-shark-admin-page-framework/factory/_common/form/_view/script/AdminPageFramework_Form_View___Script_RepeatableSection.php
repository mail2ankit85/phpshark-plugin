...
( function( $ ) {
    ...
    $.fn.updatePHPShark_AdminPageFrameworkRepeatableSections = function( aSettings ) {
        ...
        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.php-shark-sections' ).attr( 'id' );
        ...
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.php-shark-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.php-shark-section' ).length;
    ...
    $.fn.addPHPShark_AdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {
        ...
        if ( 'undefined' === typeof sSectionContainerID ) {
            var sSectionContainerID = $( this ).closest( '.php-shark-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.php-shark-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.php-shark-section-tabs-contents' )
            .children( '.php-shark-section-tabs' )
            .first();
        ...
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.php-shark-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.php-shark-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.php-shark-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.php-shark-section-title' ).not( '.php-shark-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enablePHPShark_AdminPageFrameworkCollapsibleButton ){ 
            nodeNewSection.find( '.php-shark-collapsible-sections-title, .php-shark-collapsible-section-title' ).enablePHPShark_AdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.php-shark-field' ).each( function( iFieldIndex ) {
            ...
            $( this ).trigger( 
                'php-shark_added_repeatable_field', 
                [
            ...
            // 3.8.8 
            $( this ).trigger( 
                'php-shark_repeated_field', 
                [
                    1, // call type, 0: repeatable fields, 1: repeatable sections, 
                    jQuery( nodeNewSection ).closest( '.php-shark-sections' )    // model container
                ]
            );              
        ...
        nodeNewSection.find( '.php-shark-fields.sortable' ).each( function() {
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.php-shark-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.php-shark-fieldrow, .php-shark-fieldset, .php-shark-fields, .php-shark-field, table.form-table, input,textarea,select,option' )
                .incrementAttribute( 
                    'id', 
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.php-shark-fields' ).incrementAttribute( 
                'data-field_tag_id_model',
                iSectionsCount,
                _sSectionIDModel
            );
            $( oElement ).find( '.php-shark-fields' ).incrementAttributes( 
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.php-shark-fieldset' ).incrementAttribute( 
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
    ...
    $.fn.removePHPShark_AdminPageFrameworkRepeatableSection = function() {
        ...
        var nodeSectionContainer    = $( this ).closest( '.php-shark-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.php-shark-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );        
        var nodeTabsContainer       = $( this ).closest( '.php-shark-section-tabs-contents' )
            .children( '.php-shark-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.php-shark-section-tab' );
        ...
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.php-shark-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.php-shark-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() { 
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.php-shark-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.php-shark-section-title' ).first().show();
                }                
            }            
...
var _iSectionIndex = _iIterationIndex;
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.php-shark-field' ).each( function( iFieldIndex ) {    
...
jQuery( document ).ready( function() {
    ...
    // Adds the buttons
    jQuery( '#{$sContainerTagID} .php-shark-section-caption' ).each( function(){
        
        jQuery( this ).show();
        
        var _oButtons = jQuery( $_sButtonsHTML );
        if ( jQuery( this ).children( '.php-shark-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
            _oButtons.addClass( 'section_title_field_sibling' );
        }
        var _oCollapsibleSectionTitle = jQuery( this ).find( '.php-shark-collapsible-section-title' );
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='php-shark-section-repeatable-script'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>";
    }
    static private function ___getContainerAttributes(array $aArguments, $oMsg) {
        $_aAttriubtes = array('class' => self::getClassAttribute('php-shark-repeatable-section-buttons', empty($aArguments['disabled']) ? '' : 'disabled'),);
